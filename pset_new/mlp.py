...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # output layer
    # z = np.zeros((P.shape[0], P.shape[1] - 1), dtype=y.dtype)
    # y = np.c_[y, z]
    temp = np.zeros(P.shape)
    for i in range(P.shape[0]):
      temp[i][y[i]] = 1
    dscore = np.subtract(P, temp) # [PLEASE IMPLEMENT] partial derivative of loss wrt. the logits (dL/dz)
    ...
    dz1 = np.zeros(hidden.shape)
    if self.activation == 'relu':
      dz1[z1 > 0] = 1
      dz1[z1 <= 0] = 0
    elif self.activation == 'sigmoid':
      # dz1 = (np.exp(-z1))/((np.exp(-z1) + 1 )**2)
      dz1 = (1/(1+np.exp(-z1))) * (1 - (1/(1+np.exp(-z1))))
    else:
      raise ValueError('Unknown activation type')
    # first layer
    dz1 = np.multiply(dhidden, dz1)
  ...
  def predict(self, X):
    ...
    temp = self.loss(X)
    A = np.max(temp, axis=1) # N*1
    F = np.exp(temp - A.reshape(X.shape[0], 1))  # N*C
    P = F / np.sum(F, axis=1).reshape(X.shape[0], 1)  # N*C
